    ...
    def train(self):
        ...
        for step in pbar:
            ...
            with torch.no_grad():
                ...
                teacher_alpha = teacher_alphas[..., 0:1].detach().data # [1, H, W, 1]
            ...
            if cfg.use_novel_view:
                ...
                with torch.no_grad():
                    ...
                    teacher_alpha_ = teacher_alphas[..., 0:1].detach().data # [1, H, W, 1]
                    ...
                    teacher_alpha = torch.cat([teacher_alpha, teacher_alpha_], dim=0)
            ...
            if cfg.distill:
                ...
                xyzloss = distill_loss_function(xyzs, teacher_xyzs) * cfg.distill_xyzs_lambda 
                loss += xyzloss

                mask = (depths > 0) & (teacher_depths > 0)
                depths_masked = torch.where(mask, depths, torch.zeros_like(depths))
                teacher_depths_masked = torch.where(mask, teacher_depths, torch.zeros_like(teacher_depths))
                depthloss = distill_loss_function(depths_masked, teacher_depths_masked) * cfg.distill_depth_lambda
                loss += depthloss

                quatloss = distill_loss_function(quats, teacher_quats) * cfg.distill_quats_lambda
                loss += quatloss

                scaleloss = distill_loss_function(scales, teacher_scales) * cfg.distill_scales_lambda
                loss += scaleloss

                opacityloss = distill_loss_function(opacities, teacher_opacities) * cfg.distill_opacities_lambda
                loss += opacityloss
