...
TEACHER_CKPT = '/Bean/log/gwangjin/2025/kdgs/ms_d/bicycle_depth_reinit/ckpts/ckpt_29999_rank0.pt'
TEACHER_DIR = os.path.dirname(os.path.dirname(TEACHER_CKPT))
STUDENT_DIR = os.path.join(result_dir_root, 'ms_student_v3')

# Base command for student training, updated to use student_trainer_w_ms.py.
BASE_COMMAND = (
    "python student_trainer_w_ms_v2.py distill2d"
    ...
    " --strategy.blur_threshold 0.0002"
...
def generate_commands():
    ...
    key_for_gradient = []
    key_for_gradient = ['rendered_sh_coeffs', 'means2d']
    ...
    target_samplings = [0.15, 0.2, 0.25]

    # Force use_blur_splits to always True.
    use_blur_splits = [True]
    use_novel_views = [True]  # novel view flag still affects the command, but won't be in the output dir name.
    # use_novel_views = [True, False]  # novel view flag still affects the command, but won't be in the output dir name.


    # depth reinit effect. 
    # commands.append("python ms_trainer.py msd --data_dir /Bean/data/gwangjin/2025/kdgs/360_v2/bicycle/ --data_factor 4 --result_dir /Bean/log/gwangjin/2025/kdgs/ms_d/depth_reinit_test --use_depth_reinit --sampling_factor 0.5 --disable_viewer --depth_reinit_iters 2000 --max_steps 2100 --eval_steps 1999 2001 --num_depth 500000")

    # commands.append("python ms_trainer.py msd --data_dir /Bean/data/gwangjin/2025/kdgs/360_v2/bicycle/ --data_factor 4 --result_dir /Bean/log/gwangjin/2025/kdgs/ms_d/simp_test --use_depth_reinit --sampling_factor 0.5 --disable_viewer --simp_iteration1 1500 --simp_iteration2 2000 --max_steps 2100 --eval_steps 1499 1501 1999 2001 --num_depth 500000")
    ...
    # Outer loops for blur_split, novel_view and target_sampling_ratio.
    for use_blur_split in use_blur_splits:
        for use_novel_view in use_novel_views:
            ...
            # Loop over the target sampling ratios.
            for target_sampling in target_samplings:
                base_command_with_sampling = base_command + f" --target_sampling_ratio {target_sampling}"
                ...
                for combination in combinations:
                    ...
                    cmd_base = base_command_with_sampling
                    ...
                    if distill_sh_lambda > 0 and distill_depth_lambda > 0:
                        for gradient_key in key_for_gradient:
                            for sh_mult in sh_coeffs_mults:
                                ...
                                depth_mult = sh_mult_val * 0.2
                                for grow_grad2d in grow_grad2ds:
                                    grow_grad2d = grow_grad2d * (1 + distill_colors_lambda + distill_depth_lambda + distill_xyzs_lambda + distill_quats_lambda + distill_sh_lambda)
                                    cmd = cmd_base
                                    ...
                                    output_dir_name = (
                                        ...
                                        f'_blur{use_blur_split}_novel{use_novel_view}_target{target_sampling}'
                                    )
                                    output_dir = os.path.join(STUDENT_DIR, output_dir_name)
                                    cmd_full = cmd + f' --result_dir {output_dir}'
                                    if os.path.exists(os.path.join(output_dir, 'ckpts', 'ckpt_19999_rank0.pt')):
                                        continue
                    ...
                    # Case 3: Only depth lambda > 0.
                        ...
                        for gradient_key in key_for_gradient:
                            if gradient_key in ['rendered_sh_coeffs', 'depths_and_sh']:
                                continue
                            for sh_mult in sh_coeffs_mults:
                                depth_mult = sh_mult / distill_depth_lambda * 0.2
                                for grow_grad2d in grow_grad2ds:
                                    grow_grad2d = grow_grad2d * (1 + distill_colors_lambda + distill_depth_lambda + distill_xyzs_lambda + distill_quats_lambda + distill_sh_lambda)
                                    cmd = cmd_base
                                    cmd += f' --strategy.depths_mult {depth_mult}'
                    ...
                    # Case 4: Both lambdas are zero.
                        ...
                        for gradient_key in key_for_gradient:
                            for grow in grow_grad2ds:
                                ...
                                output_dir = os.path.join(STUDENT_DIR, output_dir_name)
                                cmd_full = cmd + f' --result_dir {output_dir}'
                                if not os.path.exists(os.path.join(output_dir, 'ckpts', 'ckpt_19999_rank0.pt')):
                                    commands.append(cmd_full)
    ...
    #                     output_dir = os.path.join(STUDENT_DIR, output_dir_name)
    #                     cmd_full = cmd + f' --result_dir {output_dir}'
    #                     if not os.path.exists(os.path.join(output_dir, 'ckpts', 'ckpt_14999_rank0.pt')):
