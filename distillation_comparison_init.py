...
BASE_COMMAND = "python student_trainer.py distill2d --teacher_ckpt gsplat_teachers/bicycle_4/ckpts/ckpt_29999_rank0.pt --data_factor 4 --data_dir data/360_v2/bicycle/ --disable_viewer --strategy.grow_grad2d 0.0009 --apply_vis_on_teacher_sampling --apply_vis_on_teacher_sampling "
STUDENT_DIR = "gsplat_students_v8"
...
def main():
    ...
    # combinations.append([0.5, 0, 0.5, 0, 0])
    combinations.append([0.5, 0, 0, 0, 0])
    combinations.append([0, 0.5, 0, 0, 0])
    combinations.append([0, 0, 0.5, 0, 0])
    combinations.append([0, 0, 0, 0.5, 0])
    combinations.append([0, 0, 0, 0, 0.5])
    ...
    for combination in combinations:
        ...
        if distill_sh_lambda > 0 and distill_depth_lambda > 0:
            for gradient_key in key_for_gradient:
                ...
                for sh_mult in sh_coeffs_mults:
                    sh_mult = 2 / distill_sh_lambda
                    depth_mult = sh_mult * 0.25
                    for grow_grad2d in grow_grad2ds:
                        ...
                        output_dir_name = (
                            ...
                            f'{gradient_key}_sh{sh_mult}_depth{depth_mult}_grow2d{grow_grad2d}_novelview'
                        )
                        output_dir = os.path.join(STUDENT_DIR, output_dir_name)
                        cmd += f' --result_dir {output_dir}'
                        cmd += f' --use_novel_view'
                ...
                for sh_mult in sh_coeffs_mults:
                    ...
                    for grow_grad2d in grow_grad2ds:
                        ...
                        output_dir_name = (
                            BASIC_output_dir_name +
                            f'_sh{distill_sh_lambda}_colors{distill_colors_lambda}_depth{distill_depth_lambda}_xyzs{distill_xyzs_lambda}_quats{distill_quats_lambda}_'
                            f'{gradient_key}_sh{sh_mult}_grow2d{grow_grad2d}_novelview'
                        )
                        output_dir = os.path.join(STUDENT_DIR, output_dir_name)
                        cmd += f' --result_dir {output_dir}'
                        cmd += f' --use_novel_view'
        ...
        # Case 3: Only depth lambda > 0
            ...
            for gradient_key in key_for_gradient:
                ...
                for depth_mult in sh_coeffs_mults:
                    depth_mult = 2 / distill_depth_lambda * 0.25
                    for grow_grad2d in grow_grad2ds:
                        cmd = base_command
                        cmd += f' --strategy.depths_mult {depth_mult}'
                        cmd += f' --strategy.key_for_gradient {gradient_key}'
                        ...
                        output_dir_name = (
                            BASIC_output_dir_name +
                            f'_sh{distill_sh_lambda}_colors{distill_colors_lambda}_depth{distill_depth_lambda}_xyzs{distill_xyzs_lambda}_quats{distill_quats_lambda}_'
                            f'{gradient_key}_depth{depth_mult}_grow2d{grow_grad2d}_novelview'
                        )
                        output_dir = os.path.join(STUDENT_DIR, output_dir_name)
                        cmd += f' --result_dir {output_dir}'
                        cmd += f' --use_novel_view'
        ...
        # Case 4: Both lambdas are zero -> produce one unified command (to avoid duplicates)
            ...
            output_dir_name = (
                BASIC_output_dir_name +
                f'_sh{distill_sh_lambda}_colors{distill_colors_lambda}_depth{distill_depth_lambda}_xyzs{distill_xyzs_lambda}_quats{distill_quats_lambda}_novelview'
            )
            output_dir = os.path.join(STUDENT_DIR, output_dir_name)
            cmd += f' --result_dir {output_dir}'
            cmd += f' --use_novel_view'
    ...
    for combination in combinations:        
        ...
        for gradient_key in ['means2d']:
            ...
            for grow_grad2d in grow_grad2ds:
                ...
                output_dir_name = (
                    BASIC_output_dir_name +
                    f'_sh{distill_sh_lambda}_colors{distill_colors_lambda}_depth{distill_depth_lambda}_xyzs{distill_xyzs_lambda}_quats{distill_quats_lambda}_{gradient_key}_grow2d{grow_grad2d}_novelview'
                )
                output_dir = os.path.join(STUDENT_DIR, output_dir_name)
                cmd += f' --result_dir {output_dir}'
                cmd += f' --use_novel_view'
                ...
                output_dir_name = (
                    BASIC_output_dir_name +
                    f'_sh{distill_sh_lambda}_colors{distill_colors_lambda}_depth{distill_depth_lambda}_xyzs{distill_xyzs_lambda}_quats{distill_quats_lambda}_{gradient_key}_grow2d{grow_grad2d}'
                )
                output_dir = os.path.join(STUDENT_DIR, output_dir_name)
                cmd += f' --result_dir {output_dir}'
                if os.path.exists(os.path.join(output_dir, 'ckpts', 'ckpt_29999_rank0.pt')):
