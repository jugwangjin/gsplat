...
def parse_subdir_name(subdir_name, prefix="bicycle_4"):
    ...
    Expected naming patterns include:
      1. When all five lambdas are used:
         ...
         e.g. bicycle_4_sh0.5_colors0_depth0_xyzs0_quats0_depths_and_sh_sh4_depth4_grow2d0.0002_novelview
      2. Only sh lambda > 0:
         bicycle_4_sh{sh}_colors{colors}_depth{depth}_xyzs{xyzs}_quats{quats}_{gradient_key}_...
         e.g. bicycle_4_sh0.5_colors0_depth0_xyzs0_quats0_means2d_grow2d0.0008_novelview
      3. Both lambdas zero:
    ...
    remaining = tokens[5:]
    # Remove a trailing "novelview" token if present.
    if remaining and remaining[-1].lower() == "novelview":
    ...
    # Case 1: Both sh and depth lambdas > 0 -> expect last three tokens:
    # a token starting with "sh" (sh_coeffs_mult), one with "depth" (depths_mult), and one with "grow2d".
    ...
    # Case 2: Only sh lambda > 0 -> expect last two tokens: one with "sh" and one with "grow2d".
    elif sh_lambda > 0:
        if total_remaining >= 3:
            ...
            params["gradient_key"] = remaining[0]
    # Case 3: Only depth lambda > 0 -> expect last two tokens: one with "depth" and one with "grow2d".
    elif depth_lambda > 0:
        if total_remaining >= 3:
            gradient_key_tokens = remaining[:total_remaining - 2]
            params["gradient_key"] = "_".join(gradient_key_tokens)
...
def analyze_students_dir(students_dir, output_csv, prefix="bicycle_4"):
    ...
    A new column "novel_view" is added which is True if the subdirectory name
    contains the token "novelview", and False otherwise.
    """
    results = []
    ...
    for root, dirs, files in os.walk(students_dir):
        ...
        if os.path.exists(json_file):
            ...
            # Add the novel view flag: True if "novelview" is in the subdir name.
            combined["novel_view"] = "novelview" in subdir_name.lower()
