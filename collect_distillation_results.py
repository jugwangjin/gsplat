...
def parse_subdir_name(subdir_name, prefix="bicycle_4"):
    ...
    Expected naming patterns:
      1. Both sh and depth lambdas > 0:
         bicycle_4_sh{sh}_colors{colors}_depth{depth}_quats{quats}_{gradient_key}_sh{sh_mult}_depth{depth_mult}_grow2d{grow_grad2d}
      2. Only sh lambda > 0:
         bicycle_4_sh{sh}_colors{colors}_depth{depth}_quats{quats}_{gradient_key}_sh{sh_mult}_grow2d{grow_grad2d}
      3. Only depth lambda > 0:
         bicycle_4_sh{sh}_colors{colors}_depth{depth}_quats{quats}_{gradient_key}_depth{depth_mult}_grow2d{grow_grad2d}
      4. Both lambdas are zero:
         bicycle_4_sh{sh}_colors{colors}_depth{depth}_quats{quats}
      5. The 'means2d' case:
         bicycle_4_sh{sh}_colors{colors}_depth{depth}_quats{quats}_means2d_grow2d{grow_grad2d}
    
    This function extracts:
      - distill_sh_lambda, distill_colors_lambda, distill_depth_lambda, distill_quats_lambda
    ...
    # Extract the first four tokens: lambdas
    ...
    if tokens[3].startswith("quats"):
        params["distill_quats_lambda"] = tokens[3][5:]
    else:
