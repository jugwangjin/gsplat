...
@dataclass
class ImageAwareStrategy(DefaultStrategy):
    ...
    def initialize_state(self, scene_scale: float = 1.0) -> Dict[str, Any]:
        ...
        state = {"grad2d": None, "count": None, "scene_scale": scene_scale, "blur_mask": None, "loss_increase": None}
    ...
    def _update_state(
        ...
        if state["loss_increase"] is None:
            state["loss_increase"] = torch.zeros(n_gaussian, device=grads.device)
        ...
        for c in range(info["max_ids"].shape[0]):
            ...
            potential_loss_increase = info["accumulated_potential_loss"][c] # shape of: C, N 
            mean_potential_loss_increase = potential_loss_increase / num_pixels

            state["loss_increase"] = state["loss_increase"] + mean_potential_loss_increase
    ...
    def _grow_gs(
        ...
        # Here, we visualize the grad2d and loss_increase on 2D plot, for both is_split and is_dupli. index them. 
        if self.draw_plot:
            import matplotlib.pyplot as plt
            plt.clf()
            grad2d_full = state["grad2d"] / state["count"].clamp_min(1)
            loss_increase_full = state["loss_increase"] / state["count"].clamp_min(1)
            grad2d_split = state["grad2d"][is_split] / state["count"][is_split].clamp_min(1)
            loss_increase_split = state["loss_increase"][is_split] / state["count"][is_split].clamp_min(1)
            grad2d_dupli = state["grad2d"][is_dupli] / state["count"][is_dupli].clamp_min(1)
            loss_increase_dupli = state["loss_increase"][is_dupli] / state["count"][is_dupli].clamp_min(1)
            plt.scatter(grad2d_full.cpu().data.numpy(), loss_increase_full.cpu().data.numpy(), s=0.25, alpha=0.5, label="full")
            plt.scatter(grad2d_split.cpu().data.numpy(), loss_increase_split.cpu().data.numpy(), s=0.25, alpha=0.5, label="split")
            plt.scatter(grad2d_dupli.cpu().data.numpy(), loss_increase_dupli.cpu().data.numpy(), s=0.25, alpha=0.5, label="dupli")
            plt.xlabel("grad2d")
            plt.ylabel("loss_increase")
            plt.legend()
            plt.savefig(f"loss_increase_{info['step']}.png")
