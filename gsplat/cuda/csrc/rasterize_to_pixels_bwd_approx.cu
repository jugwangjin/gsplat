...
__global__ void rasterize_to_pixels_bwd_approx_kernel(
    const uint32_t C,
    const uint32_t N,
...
call_kernel_with_dim_approx(
    ...
    if (n_isects) {
        ...
        if (cudaFuncSetAttribute(
                rasterize_to_pixels_bwd_approx_kernel<CDIM, float>,
...
std::tuple<
    ...
    torch::Tensor,
    torch::Tensor>
rasterize_to_pixels_bwd_approx_tensor(
    // Gaussian parameters
    const torch::Tensor &means2d,                   // [C, N, 2] or [nnz, 2]
    const torch::Tensor &conics,                    // [C, N, 3] or [nnz, 3]
    const torch::Tensor &colors,                    // [C, N, 3] or [nnz, 3]
