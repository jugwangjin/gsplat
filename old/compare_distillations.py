...
from utils import AppearanceOptModule, CameraOptModule, knn, rgb_to_sh, set_random_seed, visualize_id_maps
...
from gsplat.strategy import DefaultStrategy, MCMCStrategy, DistillationStrategy
...
class Config:
    ...
    eval_steps: List[int] = field(default_factory=lambda: [500, 2_500, 5_000, 15_000])
    # Steps to save the model
    save_steps: List[int] = field(default_factory=lambda: [15_000])
    # Whether to save ply file (storage size can be large)
    save_ply: bool = False
    # Steps to save the model as ply
    ply_steps: List[int] = field(default_factory=lambda: [15_000])
    ...
    strategy: Union[DefaultStrategy, MCMCStrategy, DistillationStrategy] = field(
    ...
    distill = distill_colors_lambda > 0.0 or distill_depth_lambda > 0.0 or distill_quats_lambda > 0.0 or distill_sh0_lambda > 0.0 or distill_shN_lambda > 0.0
...
def create_splats_with_optimizers(
    ...
    # # DEBUG: save all_world_coords as pointcloud - to compare with already trained model
...
class Runner:
    ...
    def train(self):
        ...
        for step in pbar:
            ...
            param_names = ["means", "scales", "quats", "opacities", "sh0", "shN", "means2d", "depths"]

            loss_terms = {
                "l1loss": l1loss,
                "colorloss": colorloss,  # if defined, even if not added to loss
                "depthloss": depthloss,
                "quatloss": quatloss,
                "sh0loss": sh0loss,
                "shNloss": shNloss,
            ...
            grad_statistics = {}  # reset for the current step

            for loss_name, loss_val in loss_terms.items():
                grads = torch.autograd.grad(
                    loss_val, params, retain_graph=True, allow_unused=True
                )
                grad_statistics[loss_name] = {}
                for pname, grad in zip(param_names, grads):
                    if grad is not None:
                        grad_statistics[loss_name][pname] = {
                            "mean": grad.mean().item(),
                            "min": grad.min().item(),
                            "max": grad.max().item(),
                            "std": grad.std().item(),
            ...
            # Optionally, add the step number to the statistics.
            all_grad_statistics.append({
                "step": step,
                "statistics": grad_statistics
            })
            ...
            if step > 100:
                import pandas as pd
                def write_accumulated_grad_stats(all_stats, filename):
                    rows = []
                    for entry in all_stats:
                        step = entry["step"]
                        for loss_term, param_stats in entry["statistics"].items():
                ...
                # Write the accumulated statistics to CSV after training:
                write_accumulated_grad_stats(all_grad_statistics, "accumulated_grad_statistics.csv")
