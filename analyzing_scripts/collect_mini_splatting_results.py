...
def plot_learning_curve(dataset, base_dir, output_dir):
    ...
    # Filter for mini-splatting directories
    subdirectories = [d for d in subdirectories if d.endswith('_mini')]
    ...
    plt.xlabel('Step')
    plt.ylabel('PSNR')
    plt.title(f'Mini-Splatting Learning Curve for {dataset} (Step vs PSNR)')
    plt.ylim(18)
    plt.legend()
    plt.grid(True)
    
    # Save the plot
    output_image = os.path.join(output_dir, f'{dataset}_mini_learning_curve.png')
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Plot learning curves for mini-splatting experiments and copy select rendered images.")
    parser.add_argument("--base-dir", required=True, help="Base directory containing mini-splatting experiment results.")
