...
def parse_directory_name(dir_name):
    """
    Parse the directory name to extract configuration parameters from the training run.
    
    Expected detailed naming format:
    ...
    _alpha{distill_alpha_lambda}_sh{distill_sh_lambda}_colors{distill_colors_lambda}_depth{distill_depth_lambda}_xyzs{distill_xyzs_lambda}
    ...
    try:
        ...
        # Simple "zero" format fallback.
        if len(parts) == 5 and parts[3] == "zero":
            config['mode'] = 'zero'
            config['grow_grad2d'] = parts[4].replace("grow2d", "")
        # Detailed format expected to have at least 20 parts.
        elif len(parts) >= 20:
            ...
            config['distill_alpha_lambda'] = parts[8].replace("alpha", "")
            config['distill_sh_lambda']    = parts[9].replace("sh", "")
            config['distill_colors_lambda']= parts[10].replace("colors", "")
            config['distill_depth_lambda'] = parts[11].replace("depth", "")
            config['distill_xyzs_lambda']  = parts[12].replace("xyzs", "")
            config['distill_quats_lambda'] = parts[13].replace("quats", "")
            config['distill_scales_lambda']= parts[14].replace("scales", "")
            config['distill_opacities_lambda'] = parts[15].replace("opacities", "")
            config['image_lambda']         = parts[16].replace("image", "")
            config['distill_loss_terms']   = parts[17]
            config['gradient_key']         = parts[18]
            config['grow_grad2d']          = parts[19].replace("grow2d", "")
        else:
