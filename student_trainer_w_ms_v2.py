...
class Config:
    ...
    simp_iteration1: int = 5_000
    simp_iteration2: int = 10_000
    ...
    eval_steps: List[int] = field(default_factory=lambda: [1, 5_000, 20_000])
    # Steps to save the model
    save_steps: List[int] = field(default_factory=lambda: [5_000, 20_000])
    # Whether to save ply file (storage size can be large)
    save_ply: bool = False
    # Steps to save the model as ply
    ply_steps: List[int] = field(default_factory=lambda: [5_000, 20_000])
...
class Runner:
    ...
    def __init__(
        ...
        if cfg.original_initialization:
            ...
            num_depth = int(cfg.target_sampling_ratio * 4 * len(self.teacher_splats["means"]))
            target_n_gaussians = int(cfg.target_sampling_ratio * 4 * len(self.teacher_splats["means"]))
            current_n_gaussians = len(self.teacher_splats["means"])
    ...
    def train(self):
        ...
        for step in pbar:
            ...
            if cfg.distill:
                ...
                # make a mask both depths and teacher depths are larger than 0 
            ...
            if isinstance(self.cfg.strategy, DefaultStrategy) or isinstance(self.cfg.strategy, Distill2DStrategy):
                ...
                if (step+1) == cfg.simp_iteration1:
                    ...
                    sampling_factor = min(1, float(target_num_gs)  / float(current_num_gs))
                    ...
                    print("Number of GS before simplification: ", current_num_gs)
                    print("Number of GS after simplification: ", len(self.splats["means"]))
                    # Densification Strategy
                    self.cfg.strategy.check_sanity(self.splats, self.optimizers)
